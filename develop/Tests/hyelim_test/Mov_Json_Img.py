...
import shutil
import json
import Mov_All_Img as move_photo
# json 파일의 key 값과 사진의 이름을 비교해서 같으면 각각의 train/vaild/test 폴더에 넣기

FILE_PATH = 'D:/kor_dataset/write/'  # 모든 파일들이 들어있는 경로
FILE_PATH_IMG = 'dataset_all/'  # 모든 이미지 파일들이 들어있는 경로

def main():
    json_names = ['train', 'validation', 'test']
    # fold_list = os.listdir(FILE_PATH + FILE_PATH_IMG)
    
    for name in json_names:
        with open(json_names[name]+'_annotation' + '.json', 'w', encoding='UTF8') as f:
